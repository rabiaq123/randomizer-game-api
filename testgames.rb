...
class TestUseCases < Test::Unit::TestCase
  ...
  def test_uc_1
    ...
    assert({ sides: 2, up: nil, item: :coin, denomination: 0.25 } == coin1.description,
           "Coin 1's sideup value should not be set before being flipped, and should have a denomination of 0.25")
    assert({ sides: 2, up: nil, item: :coin, denomination: 0.25 } == coin2.description,
           "Coin 2's sideup value should not be set before being flipped, and should have a denomination of 0.25")
    assert({ sides: 2, up: nil, item: :coin, denomination: 0.25 } == coin3.description,
           "Coin 3's sideup value should not be set before being flipped, and should have a denomination of 0.25")
    ...
    player2.store(coin2)
    player3.store(coin3)
    puts "\t\tNum randomizers in Player 1's bag" + ' EXPECTED '.blue + '1' + ' RECEIVED: '.blue + "#{player1.bag.count}"
    puts "\t\tNum randomizers in Player 2's bag" + ' EXPECTED '.blue + '1' + ' RECEIVED: '.blue + "#{player2.bag.count}"
    puts "\t\tNum randomizers in Player 3's bag" + ' EXPECTED '.blue + '1' + ' RECEIVED: '.blue + "#{player3.bag.count}"
    ...
    puts "\t\tNum randomizers in Player 2's cup" + ' EXPECTED '.blue + '1' + ' RECEIVED: '.blue + "#{player2.cup.count}"
    ...
    puts "\t\tNum randomizers in Player 3's cup" + ' EXPECTED '.blue + '1' + ' RECEIVED: '.blue + "#{player3.cup.count}"
  ...
  def test_uc_2
    ...
    assert({ sides: 6, up: nil, item: :die, colour: :yellow } == p1_hand.randomizers[0].description,
           'Die is missing necessary attributes')
    assert_equal(p1_hand.randomizers[0].description, p1_hand.randomizers[1].description,
                 'Die 1 and Die 2 should have the same description')
    puts "\t\tNum randomizers in Player 1's hand" + ' EXPECTED: '.blue + '2;' + ' RECEIVED: '.blue + "#{p1_hand.count}"
    ...
    assert({ sides: 6, up: nil, item: :die, colour: :blue } == p2_hand.randomizers[0].description,
           'Die is missing necessary attributes')
    assert_equal(p2_hand.randomizers[0].description, p2_hand.randomizers[1].description,
                 'Die 3 and Die 4 should have the same description')
    puts "\t\tNum randomizers in Player 2's hand" + ' EXPECTED: '.blue + '2;' + ' RECEIVED: '.blue + "#{p2_hand.count}"
    assert_not_nil(p2_hand.randomizers, "Player 2's hand should have 2 dice")

    # put players' dice in their bags
    ...
    puts "\t\tNum randomizers in Player 1's hand" + ' EXPECTED: '.blue + '0;' + ' RECEIVED: '.blue + "#{p1_hand.count}"
    puts "\t\tNum randomizers in Player 2's hand" + ' EXPECTED: '.blue + '0;' + ' RECEIVED: '.blue + "#{p2_hand.count}"
    ...
    puts "\t\tNum randomizers in Player 1's bag" + ' EXPECTED: '.blue + '2;' + ' RECEIVED: '.blue + "#{player1.bag.count}"
    puts "\t\tNum randomizers in Player 2's bag" + ' EXPECTED: '.blue + '2;' + ' RECEIVED: '.blue + "#{player2.bag.count}"
    assert_equal(player1.bag.count, 2, "Player 1's bag should have 2 dice")
    assert_equal(player2.bag.count, 2, "Player 2's bag should have 2 dice")
    ...
    assert(player1.bag.randomizers[0].sideup.nil? && player1.bag.randomizers[1].sideup.nil?,
           'Dice should not have a sideup value in bag')
    assert(player2.bag.randomizers[0].sideup.nil? && player2.bag.randomizers[1].sideup.nil?,
           'Dice should not have a sideup value in bag')

    # transfer dice from players' bags to the cup
    puts "\tTransferring each player's dice from their bag to their cup"
    player1.load
    player2.load
    # ensure randomizers are no longer in bag
    puts "\t\tNum randomizers in Player 1's bag" + ' EXPECTED: '.blue + '0;' + ' RECEIVED: '.blue + "#{player1.bag.count}"
    assert_equal(player1.bag.randomizers, [], "Player 1's bag should be empty")
    puts "\t\tNum randomizers in Player 2's bag" + ' EXPECTED: '.blue + '0;' + ' RECEIVED: '.blue + "#{player2.bag.count}"
    assert_equal(player2.bag.randomizers, [], "Player 2's bag should be empty")
    # ensure randomizers are in cup
    puts "\t\tNum randomizers in Player 1's cup" + ' EXPECTED: '.blue + '2;' + ' RECEIVED: '.blue + "#{player1.cup.count}"
    assert_not_equal(player1.cup.randomizers, [], "Player 1's cup should not be empty")
    puts "\t\tNum randomizers in Player 2's cup" + ' EXPECTED: '.blue + '2;' + ' RECEIVED: '.blue + "#{player2.cup.count}"
    ...
    puts "\t\tplayer1.sum({})" + ' EXPECTED: '.blue + "#{[die1.sideup + die2.sideup]}" + ' RECEIVED: '.blue + "#{player1.sum({})}"
    assert_equal(player1.sum({}), [die1.sideup + die2.sideup],
                 'player1.sum({}) should return an array of the sum of all die roll values for each cup throw')
    puts "\t\tplayer2.sum({})" + ' EXPECTED: '.blue + "#{[die3.sideup + die4.sideup]}" + ' RECEIVED: '.blue + "#{player2.sum({})}"
    assert_equal(player2.sum({}), [die3.sideup + die4.sideup],
                 'player2.sum({}) should return an array of the sum of all die roll values for each cup throw')
  end
  ...
  def test_uc_3
    ...
    puts "\tCreating die for Player 1 to store in bag"
    ...
    assert({ sides: 6, up: nil, item: :die, colour: :yellow } == die.description,
           'Die is missing necessary attributes')
    ...
    p1_hand = Hand.new
    p1_hand.store(die)
    puts "\t\tNum randomizers in Player 1's hand" + ' EXPECTED: '.blue + '1;' + ' RECEIVED: '.blue + "#{p1_hand.count}"
    assert(p1_hand.count == 1, "Player 1's hand should have 1 die")
    ...
    puts "\t\tNum randomizers in Player 1's hand" + ' EXPECTED: '.blue + '0;' + ' RECEIVED: '.blue + "#{p1_hand.count}"
    puts "\t\tNum randomizers in Player 1's bag" + ' EXPECTED: '.blue + '1;' + ' RECEIVED: '.blue + "#{player1.bag.count}"
    assert_equal(player1.bag.count, 1, "Player 1's bag should have 1 die")
    ...
    while player1_turn
      ...
      puts "\t\tNum die rolls" + ' EXPECTED: '.blue + "#{num_calls};" + ' RECEIVED: '.blue + "#{die.calls}"
    ...
    p2_hand = Hand.new
    p2_hand.store(die)
    puts "\t\tNum randomizers in Player 1's cup" + ' EXPECTED: '.blue + '0;' + ' RECEIVED: '.blue + "#{player1.cup.count}"
    assert(player1.cup.randomizers == [] && p1_hand.randomizers == [], "Player 1's hand and cup should be empty")
    puts "\t\tNum randomizers in Player 2's hand" + ' EXPECTED: '.blue + '1;' + ' RECEIVED: '.blue + "#{p2_hand.count}"
    assert_equal(p2_hand.count, 1, "Player 2's hand should have 1 die")
  end
  ...
  def test_uc_4
    ...
    assert({ sides: 5, up: nil, item: :die, colour: :green } == die1.description,
           'Die is missing necessary attributes')
    assert({ sides: 5, up: nil, item: :die, colour: :blue } == die2.description,
           'Die is missing necessary attributes')
    assert({ sides: 5, up: nil, item: :die, colour: :red } == die3.description,
           'Die is missing necessary attributes')
    assert({ sides: 5, up: nil, item: :die, colour: :yellow } == die4.description,
           'Die is missing necessary attributes')
    ...
    puts "\t\tNum randomizers in Player 1's hand" + ' EXPECTED: '.blue + '4;' + ' RECEIVED: '.blue + "#{p1_hand.count}"
    assert(p1_hand.count == 4, "Player 1's hand should have 4 dice")
    ...
    # ensure randomizer is in bag and no longer in hand
    puts "\t\tNum randomizers in Player 1's hand" + ' EXPECTED: '.blue + '0;' + ' RECEIVED: '.blue + "#{p1_hand.count}"
    puts "\t\tNum randomizers in Player 1's bag" + ' EXPECTED: '.blue + '4;' + ' RECEIVED: '.blue + "#{player1.bag.count}"
    assert_equal(p1_hand.randomizers, [], "Player 1's hand should be empty after moving dice to bag")
    assert_equal(player1.bag.count, 4, "Player 1's bag should have 4 dice")
    ...
    puts "\t\tNum randomizers in Player 1's cup" + ' EXPECTED: '.blue + '1;' + ' RECEIVED: '.blue + "#{player1.cup.count}"
    assert_equal(player1.bag.count, 3, "Player 1's bag should have 3 dice")
    assert_equal(player1.cup.count, 1, "Player 1's cup should have 1 die")
    ...
    if player1.cup.randomizers[0].colour == die_criteria[:colour]
      ...
      assert(
        player1.cup.randomizers[0].sideup >= 1 && player1.cup.randomizers[0].sideup <= player1.cup.randomizers[0].sides, "Die should have a sideup value between 1 and #{player1.cup.randomizers[0].sides}"
      )
      # get results of Player 1's roll
    ...
    if tie_broken == false
      ...
      puts "\t\tNum randomizers in shared bag" + ' EXPECTED: '.blue + '4;' + ' RECEIVED: '.blue + "#{player1.bag.count}"
      assert_equal(player1.cup.count, 0, "Player 1's cup should be empty")
      assert_equal(player1.bag, player2.bag, 'Player 1 and Player 2 should have the same bag')
      assert_equal(player1.bag.count, 4, "Player 1's bag should have 4 dice")
      assert_equal(player2.bag.count, 4, "Player 2's bag should have 4 dice")
    end
  end
end
...
class TestUnusedMethods < Test::Unit::TestCase
  ...
  def test_player_methods
    ...
    # put coins in Player's bag
    puts "\tPutting coins in Player's bag"
    player.store(coin1)
    player.store(coin2)
    # transfer coins from player's bag to their cup
    ...
    3.times do
      puts "\t\tFlipping coins..."
      player.throw
      player.cup.randomizers.each do |coin|
        puts "\t\t#{coin.description}"
      end
    end
    ...
    # get results of all coin flips
    puts "\tGetting results of all coin flips"
    puts "\t\tplayer.results({}, 0)" + ' EXPECTED: '.blue + "#{result_values}" + ' RECEIVED: '.blue + "#{player.results(
      {}, 0
    )}"
    assert_equal(player.results({}, 0), result_values,
                 'player.results({}. 0) should return an array of all coin flip values for the last coin flip')
  end
  ...
  def test_bag_methods
    ...
    puts "\t\tNum randomizers in bag" + ' EXPECTED: '.blue + '2' + ' RECEIVED: '.blue + "#{player.bag.count}"
  ...
  def test_hand_methods
    ...
    puts "\t\tNum randomizers in hand" + ' EXPECTED: '.blue + '2' + ' RECEIVED: '.blue + "#{hand.count}"
    assert_equal(hand.count, 2, 'Hand should have 2 randomizers in it')

    # remove last added coin from Hand
    puts "\tRemoving last added coin from hand"
    removed_coin = hand.next
    puts "\t\tNum randomizers in hand" + ' EXPECTED: '.blue + '1' + ' RECEIVED: '.blue + "#{hand.count}"
    assert_equal(hand.count, 1, 'Hand should have 1 randomizer in it')
    puts "\t\tRemoved coin description" + ' EXPECTED: '.blue + "#{coin2.description}" + ' RECEIVED: '.blue + "#{removed_coin.description}"
    assert_equal(removed_coin.description, coin2.description,
                 'Removed coin should be the last coin added to the hand')
  end
end
