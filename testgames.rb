...
class TestUseCases < Test::Unit::TestCase
    ...
    def test_uc_2
        ...
        assert_equal(player1.sum({}), [die1.sideup + die2.sideup], "player1.sum({}) should return an array of the sum of all die roll values for each cup throw")
        puts "\t\tplayer2.sum({})" + " EXPECTED: ".blue + "#{[die3.sideup + die4.sideup]}" + " RECEIVED: ".blue + "#{player2.sum({})}"
        assert_equal(player2.sum({}), [die3.sideup + die4.sideup], "player2.sum({}) should return an array of the sum of all die roll values for each cup throw")
    end
